...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"GARMIN_CONNECT_WORKOUTS_CONFIG",
		"GARMIN_CONNECT_WORKOUTS_CONFIG_DIR",
		"GARMIN_CONNECT_WORKOUTS_DATA_DIR",
		"GARMIN_CONNECT_WORKOUTS_STATE_DIR",
		"GARMIN_CONNECT_WORKOUTS_CACHE_DIR",
		"GARMIN_CONNECT_WORKOUTS_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
	...
	return home, filepath.Join(home, ".config", "garmin-connect-workout-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "garmin-connect-workout-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("GARMIN_CONNECT_ACCESS_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "garmin-connect-workout-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "garmin-connect-workout-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "garmin-connect-workout-cli", "credentials.toml")
