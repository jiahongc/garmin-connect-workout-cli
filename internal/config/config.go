...
type Config struct {
	...
	GarminConnectAccessToken string          `toml:"connect_access_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("GARMIN_CONNECT_ACCESS_TOKEN"); v != "" {
		cfg.GarminConnectAccessToken = v
		cfg.markEnvOverride("GarminConnectAccessToken")
		cfg.AuthSource = "env:GARMIN_CONNECT_ACCESS_TOKEN"
		cfg.CredentialSource = "env:GARMIN_CONNECT_ACCESS_TOKEN"
	}
	...
	if v := os.Getenv("GARMIN_CONNECT_WORKOUTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GARMIN_CONNECT_WORKOUTS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "garmin-connect-workout-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GarminConnectAccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GARMIN_CONNECT_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"connect_access_token":        c.GarminConnectAccessToken,
			"GARMIN_CONNECT_ACCESS_TOKEN": c.GarminConnectAccessToken,
			"token":                       c.GarminConnectAccessToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		GarminConnectAccessToken: c.GarminConnectAccessToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.GarminConnectAccessToken = creds.GarminConnectAccessToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GarminConnectAccessToken = ""
	delete(c.envOverrides, "GarminConnectAccessToken")
	c.updateFileConfigField("GarminConnectAccessToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GarminConnectAccessToken"] {
			out.GarminConnectAccessToken = c.fileConfig.GarminConnectAccessToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GarminConnectAccessToken":
		c.fileConfig.GarminConnectAccessToken = c.GarminConnectAccessToken
	}
}
