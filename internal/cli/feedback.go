...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GARMIN_CONNECT_WORKOUTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GARMIN_CONNECT_WORKOUTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "garmin-connect-workout-cli/feedback")
...
When ` + "`GARMIN_CONNECT_WORKOUTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GARMIN_CONNECT_WORKOUTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "garmin-connect-workout-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  garmin-connect-workout-cli feedback list
  garmin-connect-workout-cli feedback list --limit 5
  garmin-connect-workout-cli feedback list --json`,
