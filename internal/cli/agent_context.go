...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		{
			Name:        "GARMIN_CONNECT_ACCESS_TOKEN",
			Kind:        "per_call",
			Required:    true,
			Sensitive:   true,
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "garmin-connect-workout-cli",
...
func collectAgentCommands(c *cobra.Command) []agentContextCommand {
	...
	for _, sub := range children {
		...
		// Surface Cobra annotations (e.g., api:endpoint, agent:read-only) so
