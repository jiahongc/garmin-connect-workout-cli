...
Resource scoping:
  ...
  garmin-connect-workout-cli sync

  # Sync specific resources only
  garmin-connect-workout-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  garmin-connect-workout-cli sync --full

  # Incremental sync: only records from the last 7 days
  garmin-connect-workout-cli sync --since 7d

  # Parallel sync with 8 workers
  garmin-connect-workout-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  garmin-connect-workout-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"workouts": "/workout-service/workout",
	}
