...
In local mode: searches locally synced data only.`,
  ...
  garmin-connect-workout-cli search "error timeout"

  # Force local search only
  garmin-connect-workout-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  garmin-connect-workout-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  garmin-connect-workout-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'garmin-connect-workout-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
