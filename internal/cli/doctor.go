...
import (
	...
	"garmin-connect-workout-cli/internal/store"
...
// first leaf that (a) carries the `api:endpoint` annotation, so it actually
...
func isSuggestableReadLeaf(cmd *cobra.Command) bool {
	...
	// supposed to avoid.
	if cmd.Annotations["api:endpoint"] == "" {
		return false
	}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  garmin-connect-workout-cli doctor --json
  garmin-connect-workout-cli doctor --fail-on warn
  garmin-connect-workout-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: garmin-connect-workout-cli auth set-token <token> or export GARMIN_CONNECT_ACCESS_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("GARMIN_CONNECT_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GARMIN_CONNECT_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GARMIN_CONNECT_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/workout-service/workout/types"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "garmin-connect-workout-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("garmin-connect-workout-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'garmin-connect-workout-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'garmin-connect-workout-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'garmin-connect-workout-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'garmin-connect-workout-cli sync' to refresh."
	}
	return report
}
