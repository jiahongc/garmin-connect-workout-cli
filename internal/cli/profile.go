...
func legacyProfileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".garmin-connect-workout-cli")
	return filepath.Join(dir, "profiles.json"), nil
}
...
captured: they control profile/config resolution and would never apply).`,
		Example: `  garmin-connect-workout-cli profile save my-defaults --json --compact
  garmin-connect-workout-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  garmin-connect-workout-cli profile use my-defaults
  garmin-connect-workout-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  garmin-connect-workout-cli profile list
  garmin-connect-workout-cli profile list --json`,
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  garmin-connect-workout-cli profile show my-defaults
  garmin-connect-workout-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  garmin-connect-workout-cli profile delete my-defaults --yes
  garmin-connect-workout-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
