...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  garmin-connect-workout-cli workflow archive

  # Full re-archive (ignore previous sync state)
  garmin-connect-workout-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  garmin-connect-workout-cli workflow status

  # Show status as JSON
  garmin-connect-workout-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("garmin-connect-workout-cli")
			}
