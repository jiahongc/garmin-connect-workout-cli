...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Garmin Connect Workouts CLI — Create Garmin-ready running workouts from plain English, preview them safely, then upload and schedule only when approv…`,
		Long: `Garmin Connect Workouts CLI — Create Garmin-ready running workouts from plain English, preview them safely, then upload and schedule only when approv…

Highlights (not in the official API docs):
  • workouts plan   Turn a plain-English running workout into Garmin-compatible workout steps without writing to Garmin.
  • workouts apply   Upload or update a generated workout only after showing the exact payload diff.
  • plan race-backward   Build and schedule a training block backwards from a race date.
  • history search   Find prior authored workouts by prompt, pace target, interval shape, date, or Garmin ID.
  • workouts sync-check   Check whether a workout is uploaded, scheduled, and ready for normal Garmin device sync.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'garmin-connect-workout-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'garmin-connect-workout-cli profile list')")
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if _, _, _, err := garminsession.Apply(cfg); err != nil {
		return nil, configErr(err)
	}
